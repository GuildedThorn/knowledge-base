...
- **GuildedThorn.com application data** on `websites` — the biggest hole:
  - **MongoDB** (accounts, guestbook, gallery metadata) — the connection is injected via the `guildedthorn_env` sops blob; where Mongo actually runs and whether it is dumped anywhere is **undocumented**. Resolve this first.
...
- **TrueNAS media** (CIFS `//…/media`) — primary copy only; no documented second copy.
...
- Decide whether the NAS runs **ZFS snapshots + scrubs** on the media/backup datasets, and on what schedule — currently unknown (its actual dataset/snapshot config was never captured; see [[03 Devices/TrueNAS|TrueNAS]]).
...
- [ ] Capture the NAS's actual dataset / snapshot / scrub config (fills the [[03 Devices/TrueNAS|TrueNAS]] stub)
