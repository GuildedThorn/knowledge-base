...
- **Wakatime**: genuinely configured, not just installed — `wakatime-cli` is a package on `nixos`, and [[04 Software/NixVim|NixVim]] has `wakatime.enable = true` as a real plugin, so coding time is actively tracked from the editor. This corrects the earlier assumption in [[08 Improvements/Ideas Backlog|Ideas Backlog]] that Wakatime had "no declared config" — it does, just from NixVim rather than a standalone module.
